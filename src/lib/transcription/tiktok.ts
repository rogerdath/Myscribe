...
export async function transcribeTikTok(url: string): Promise<TranscriptResult> {
  if (!process.env.OPENAI_API_KEY) {
    throw new TranscriptError(
      'TRANSCRIPTION_FAILED',
      'Audio transcription is not configured. Set OPENAI_API_KEY to enable TikTok transcription.'
    )
  }
